...
export default function GameOver({ room, myPlayer, onRestart }: GameOverProps) {
  ...
  return (
    ...
    >
      ...
      {isWinner && (
        <p className="text-amber-500 text-center mb-2 font-bold text-xl font-arabic" dir="rtl">
          مبروك للفائزين! 🎉
        </p>
      )}
      ...
      <div className="w-full space-y-4">
        <h3 className="text-xl font-semibold text-slate-700 mb-4 border-b border-slate-200 pb-2">Final Standings</h3>
              ...
              <div className="flex items-center gap-4 text-left">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm mt-1">
                    Real Name: <span className="font-semibold text-slate-700">{player.realName}</span>
                  </span>
                  <span className="text-xs text-slate-500 truncate max-w-[200px] mt-1">Bio: {player.bio}</span>
                </div>
              </div>
              {isPlayerWinner ? (
      ...
      <div className="mt-10 p-6 bg-slate-50 rounded-2xl border border-slate-200 w-full text-center shadow-sm">
        <p className="text-slate-500 mb-4">Want to play again?</p>
