      ...
      <div className="mt-10 p-6 bg-slate-50 rounded-2xl border border-slate-200 w-full text-center shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4">
          {myPlayer?.isHost && (
            ...
            >
              العب مرة أخرى (نفس الغرفة)
            </button>
          )}
          <button
            onClick={() => {
              sessionStorage.setItem('flan_rejoin_room_id', room.id);
              sessionStorage.removeItem('flan_room_id');
              window.location.reload();
            }}
            className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-3 px-8 rounded-xl transition-colors shadow-sm"
          >
            العب بشخصية جديدة
          </button>
          <button
