...
export default function Elimination({ room, myPlayer, timer, onSelectTarget, onGuess }: EliminationProps) {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-3xl font-bold text-slate-900 mb-2 text-center">مرحلة الإقصاء</h2>
      <p className="text-slate-500 text-center mb-8 max-w-md">
        {isDetective 
          ? "أنت المحقق! اختر هدفاً وخمن هويته الحقيقية." 
          : `${detectivePlayer?.fakeNickname} هو المحقق ويقوم بالتخمين الآن...`}
      </p>
      ...
      {isDetective ? (
        <div className="w-full">
          <h3 className="text-lg font-medium text-slate-700 mb-4">اختر الهدف:</h3>
          ...
          {room.targetId && (
            <form onSubmit={handleGuessSubmit} className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm">
              <label className="block text-sm font-medium text-slate-500 mb-2">
                خمن الاسم الحقيقي لـ {targetPlayer?.fakeNickname}:
              </label>
              <div className="flex gap-3">
                ...
                >
                  ...
                  {room.allRealNames && room.allRealNames.map((name, idx) => (
          ...
          {room.targetId ? (
            <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 shadow-sm">
              <p className="text-xl text-slate-700 mb-2">
                <span className="font-bold text-amber-500">{detectivePlayer?.fakeNickname}</span> اختار <span className="font-bold text-red-500">{targetPlayer?.fakeNickname}</span> كهدف!
              </p>
              <p className="text-slate-500 animate-pulse">في انتظار التخمين...</p>
            </div>
          ) : (
            <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 shadow-sm">
              <p className="text-slate-500 animate-pulse">في انتظار {detectivePlayer?.fakeNickname} لاختيار هدف...</p>
            </div>
          )}
        </div>
      )}
